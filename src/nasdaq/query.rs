//! List sort/filter helpers for Nasdaq RSS items (no satellite tables).
...
use super::{
    ...
    feeds::{NasdaqExtraField, NasdaqFeedKind},
};
...
pub fn list_select(kind: NasdaqFeedKind) -> Select<item::Entity> {
...
pub fn apply_sort(
    ...
    if kind
        .extra_list_fields()
        .iter()
        .any(|f| matches!(f, NasdaqExtraField::Category))
        && let Some(desc) = sort_direction(sort, NasdaqExtraField::Category.sort_key())
    {
...
pub fn apply_filters(
    ...
    if kind
        ...
        .iter()
        .any(|f| matches!(f, NasdaqExtraField::Category))
        && let Some(v) = filter_value(filters, NasdaqExtraField::Category.sort_key())
    {
...
pub fn filter_fields(kind: NasdaqFeedKind) -> Vec<FilterField> {
    let mut fields = vec![
        FilterField {
            key: "Title",
            label: "Title",
