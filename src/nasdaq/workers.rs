...
use super::{NasdaqTag, fetch, state::NasdaqState};
...
where
    M: GetByTag<NasdaqTag, Idx, Value = NasdaqState> + Sync,
{
    async fn run_serve_startup(app: &MountedApp<M>) -> anyhow::Result<()> {
        let state = app.get_capability_output::<NasdaqTag, Idx>().clone();
        spawn_poller(state);
        Ok(())
    }
}

pub fn spawn_poller(state: NasdaqState) {
    tokio::spawn(async move {
        run_poll_loop(state).await;
    });
}

async fn run_poll_loop(state: NasdaqState) {
    let interval = Duration::from_secs(state.config.poll_interval_secs.max(60));
    loop {
        if let Err(e) = fetch::fetch_all(&state).await {
            warn!(error = %e, "Nasdaq poll cycle failed");
        }
