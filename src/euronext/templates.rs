...
use super::feeds::EuronextFeedKind;
...
fn euronext_menu(current_path: &str) -> Markup {
    let feed_meta: Vec<(&'static str, &'static str, String)> = EuronextFeedKind::ALL
        .iter()
...
impl RenderAppPane for FeedItemListPage {
    fn render_pane(&self) -> lariv_rs::components::AppLayoutHtml {
        scaffold_pane(
            euronext_menu(&self.path_and_query),
...
impl RenderTemplate for FeedItemListPage {
    fn render(&self, chrome: &ShellChrome) -> Markup {
        app_scaffold(
            &format!("{} — Euronext", self.feed_name),
            chrome,
            euronext_menu(&self.path_and_query),
            feed_list_crumbs(&self.feed_name),
            self.render_table(),
...
impl RenderAppPane for FeedItemDetailPage {
    fn render_pane(&self) -> lariv_rs::components::AppLayoutHtml {
        scaffold_pane(
            euronext_menu(&feed_list_url(&self.feed_slug)),
