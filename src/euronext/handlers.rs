...
use super::{
    ...
    feeds::{EuronextFeedKind, item_document_link},
    fetch,
    keys::ItemTableKey,
    query,
...
pub async fn list(
    ...
    let Some(kind) = EuronextFeedKind::from_slug(&feed) else {
        ...
        .map(|m| {
            let extra = extra_columns
                ...
                .map(|col| match col.sort_key {
                    "Ticker" => truncate(m.company_ticker.as_deref().unwrap_or(""), 140),
                    "Company" => truncate(m.company_name.as_deref().unwrap_or(""), 140),
                    _ => String::new(),
                })
                .collect();
            FeedItemRow {
...
pub async fn detail(
    ...
    if let Some(ticker) = item.company_ticker.as_deref().filter(|s| !s.is_empty()) {
        extra_fields.push(("Ticker".to_string(), ticker.to_string()));
    }
    if let Some(company) = item.company_name.as_deref().filter(|s| !s.is_empty()) {
        extra_fields.push(("Company".to_string(), company.to_string()));
    }
    if let Some(attachment) = item.attachment.as_deref().filter(|s| !s.is_empty()) {
        extra_fields.push(("Attachment".to_string(), attachment.to_string()));
    }
    if let Some(title) = item.attachment_title.as_deref().filter(|s| !s.is_empty()) {
        extra_fields.push(("Attachment title".to_string(), title.to_string()));
    }
...
pub async fn refresh(
    ...
    if let Err(e) = fetch::fetch_feed(&state, kind).await {
        tracing::warn!(feed = kind.slug(), error = %e, "manual Euronext refresh failed");
    }
