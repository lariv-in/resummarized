...
fn jpx_menu(current_path: &str) -> Markup {
    let feed_meta: Vec<(&'static str, &'static str, String)> = JpxFeedKind::ALL
        .iter()
...
impl RenderAppPane for FeedItemDetailPage {
    fn render_pane(&self) -> lariv_rs::components::AppLayoutHtml {
        scaffold_pane(
            jpx_menu(&feed_list_url(&self.feed_slug)),
