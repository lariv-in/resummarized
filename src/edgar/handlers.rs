...
use super::{
    ...
    feeds::{EdgarFeedKind, item_document_link},
    fetch,
    keys::ItemTableKey,
    query,
...
pub async fn list(
    ...
    let Some(kind) = EdgarFeedKind::from_slug(&feed) else {
...
pub async fn detail(
    ...
    if let Some(category) = item.category.as_deref().filter(|s| !s.is_empty()) {
        extra_fields.push(("Form Type".to_string(), category.to_string()));
    }
    ...
    if let Some(guid) = item.guid.as_deref().filter(|s| !s.is_empty()) {
        extra_fields.push(("Accession / ID".to_string(), guid.to_string()));
    }
...
pub async fn refresh(
    ...
    if let Err(e) = fetch::fetch_feed(&state, kind).await {
        tracing::warn!(feed = kind.slug(), error = %e, "manual EDGAR refresh failed");
    }
