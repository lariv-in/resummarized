...
fn edgar_menu(current_path: &str) -> Markup {
    let feed_meta: Vec<(&'static str, &'static str, String)> = EdgarFeedKind::ALL
        .iter()
...
impl RenderAppPane for FeedItemListPage {
    fn render_pane(&self) -> lariv_rs::components::AppLayoutHtml {
        scaffold_pane(
            edgar_menu(&self.path_and_query),
...
impl RenderTemplate for FeedItemListPage {
    fn render(&self, chrome: &ShellChrome) -> Markup {
        app_scaffold(
            &format!("{} — SEC EDGAR", self.feed_name),
            chrome,
            edgar_menu(&self.path_and_query),
            feed_list_crumbs(&self.feed_name),
            self.render_table(),
            ...
            (container_column(
                ...
                html! {
                    ...
                    @if !self.description.is_empty() {
                        (label("Summary", field_text(FieldText {
...
impl RenderAppPane for FeedItemDetailPage {
    fn render_pane(&self) -> lariv_rs::components::AppLayoutHtml {
        scaffold_pane(
            edgar_menu(&feed_list_url(&self.feed_slug)),
