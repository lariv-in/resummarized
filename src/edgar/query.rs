//! List sort/filter helpers for SEC EDGAR items.
...
use super::{
    ...
    feeds::{EdgarExtraField, EdgarFeedKind},
};
...
pub fn list_select(kind: EdgarFeedKind) -> Select<item::Entity> {
...
pub fn apply_sort(
    ...
    if kind
        .extra_list_fields()
        .iter()
        .any(|f| matches!(f, EdgarExtraField::Category))
        && let Some(desc) = sort_direction(sort, EdgarExtraField::Category.sort_key())
    {
        return order(query, item::Column::Category, desc);
    }
...
pub fn apply_filters(
    ...
    if kind
        ...
        .iter()
        .any(|f| matches!(f, EdgarExtraField::Category))
        && let Some(v) = filter_value(filters, EdgarExtraField::Category.sort_key())
    {
        query = apply_text_contains(query, item::Column::Category, v);
    }
    query
}

pub fn filter_fields(kind: EdgarFeedKind) -> Vec<FilterField> {
    let mut fields = vec![
        FilterField {
            key: "Title",
            label: "Title",
...
mod tests {
    ...
    fn filter_fields_include_base_columns() {
        for kind in EdgarFeedKind::ALL {
            ...
            assert!(fields.iter().any(|f| f.key == "PubDate"));
            assert!(fields.iter().any(|f| f.key == "Category"));
        }
    }
}
